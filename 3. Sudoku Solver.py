        ...
        for column in range(len(board)):
            if board[row][column] == 0:
                ...
                for content in range(1,10):
                    # goes through each number, one by one
                    # checks if any could be inputted
                    # while adhering to sudoku rules
                    if check_input(row, column, content) == True:
                ...
                return # tries the next content in range(1,10)
...
def check_input(row, column, content):  
    ...
    for row_in_grid in (range(3)): # locks the row within grid
        for column_in_grid in (range(3)):
            if board[grid_row + row_in_grid] \
                [grid_column + column_in_grid] \
                == content:
                return False
